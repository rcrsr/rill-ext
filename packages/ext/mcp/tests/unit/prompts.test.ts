...
describe('Prompt Function Generation', () => {
  ...
  describe('IR-5: Generated prompt function signature', () => {
    it('generates function with correct parameters', () => {
      ...
      expect(functions).toHaveProperty('code_review');
      const fn = functions.code_review!;
    ...
    it('generates function with no parameters for prompt without arguments', () => {
      ...
      expect(functions).toHaveProperty('greeting');
      const fn = functions.greeting!;
  ...
  describe('IR-5: Calls MCP prompts/get with name and arguments', () => {
    it('calls client.getPrompt with prompt name and arguments dict', async () => {
      ...
      const functions = generatePromptFunctions(prompts, mockClient, timeoutMs);
      await functions.test_prompt!.fn(
    ...
    it('omits optional arguments when not provided', async () => {
      ...
      const functions = generatePromptFunctions(prompts, mockClient, timeoutMs);
      await functions.test!.fn(
        { required: 'req-value', optional: undefined },
        makeRuntimeCtx()
      );
    ...
    it('throws for missing required arguments', async () => {
      ...
      await expectRejectsInvalid(
        functions.test!.fn({ required: undefined }, makeRuntimeCtx()),
    ...
    it('validates argument types are strings', async () => {
      ...
      await expectRejectsInvalid(
        functions.test!.fn({ arg: 123 }, makeRuntimeCtx()),
        'expected string for parameter arg, got number'
      );
    });
  });
  ...
  describe('Name sanitization', () => {
    it('sanitizes prompt names to valid rill identifiers', () => {
      ...
      expect(functions).toHaveProperty('code_review');
      expect(functions).toHaveProperty('fetch_data');
      expect(functions).toHaveProperty('camel_case_name');
    });
    ...
    it('handles name collisions with numeric suffixes', () => {
      ...
      expect(functions).toHaveProperty('test_name');
      expect(functions).toHaveProperty('test_name_2');
      expect(functions).toHaveProperty('test_name_3');
    });
  });
  ...
  describe('Error handling', () => {
    ...
    it('handles connection lost errors', async () => {
      ...
      await expectRejectsInvalid(
        functions.test!.fn({}, makeRuntimeCtx()),
        'connection lost'
      );
    });
  ...
  describe('Multiple prompts', () => {
    it('generates functions for multiple prompts', () => {
      ...
      expect(functions).toHaveProperty('prompt1');
      expect(functions).toHaveProperty('prompt2');
      expect(functions).toHaveProperty('prompt3');
    });

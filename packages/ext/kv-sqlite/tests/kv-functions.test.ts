...
describe('kv functions', () => {
  describe('get() - IR-1', () => {
    ...
    it('returns schema default if key missing (EC-1)', () => {
      ...
      const ext = createSqliteKvExtension(config, makeFactoryCtx());
      const result = getCallable(ext, 'get').fn({
        mount: 'test',
        key: 'count',
      });
    ...
    it('throws if key not in schema (declared mode)', () => {
      ...
      expect(() =>
        getCallable(ext, 'get').fn({ mount: 'test', key: 'age' })
  ...
  describe('get_or() - IR-2', () => {
    it('returns value when key exists', () => {
      ...
      const result = getCallable(ext, 'get_or').fn({
        mount: 'test',
        key: 'name',
        fallback: 'fallback',
      });
  ...
  describe('set() - IR-3', () => {
    it('sets value and returns true', () => {
      ...
      const result = getCallable(ext, 'set').fn({
        mount: 'test',
        key: 'name',
        value: 'Charlie',
      });
  ...
  describe('merge() - IR-4', () => {
    it('merges partial dict into existing dict', () => {
      ...
      const result = getCallable(ext, 'merge').fn({
        mount: 'test',
        key: 'user',
        partial: { age: 31, city: 'NYC' },
      });
  ...
  describe('delete() - IR-5', () => {
    it('deletes existing key and returns true', () => {
      ...
      const result = getCallable(ext, 'delete').fn({
        mount: 'test',
        key: 'name',
      });

      expect(result).toBe(true);
      expect(getCallable(ext, 'has').fn({ mount: 'test', key: 'name' })).toBe(
        false
      );
  ...
  describe('has() - IR-7', () => {
    ...
    it('returns false when key does not exist', () => {
      ...
      const result = getCallable(ext, 'has').fn({
        mount: 'test',
        key: 'missing',
      });
  ...
  describe('clear() - IR-8', () => {
    ...
    it('restores schema defaults in declared mode', () => {
      ...
      expect(getCallable(ext, 'get').fn({ mount: 'test', key: 'name' })).toBe(
        'Anonymous'
      );
      expect(getCallable(ext, 'get').fn({ mount: 'test', key: 'count' })).toBe(
        0
      );

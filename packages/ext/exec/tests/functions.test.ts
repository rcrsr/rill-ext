...
describe('command execution', () => {
  it('executes command and returns stdout, stderr, exitCode', async () => {
    ...
    const result = await ext.value.echo.fn({ args: ['hello', 'world'] }, makeRuntimeCtx());
  ...
  it('merges command env with inherited env', async () => {
    ...
    const result = await ext.value.printenv.fn({ args: ['CUSTOM_VAR'] }, makeRuntimeCtx());
...
describe('runner: argument validation', () => {
  it('allows arguments in allowlist', async () => {
    ...
    const result = await runCommand('echo', config, ['hello', 'world'], undefined, undefined, makeRuntimeCtx());
  ...
  it('allows arguments not in blocklist', async () => {
    ...
    const result = await runCommand('echo', config, ['hello'], undefined, undefined, makeRuntimeCtx());
    expect((result as { exitCode: number }).exitCode).toBe(0);
  });
...
describe('runner: error handling', () => {
  it('returns non-zero exit code without error', async () => {
    const result = await runCommand('sh', { binary: 'sh' }, ['-c', 'exit 42'], undefined, undefined, makeRuntimeCtx());
    expect((result as { exitCode: number }).exitCode).toBe(42);
  });

  it('captures stderr output', async () => {
    const result = await runCommand('sh', { binary: 'sh' }, ['-c', 'echo error >&2; exit 1'], undefined, undefined, makeRuntimeCtx());
...
describe('runner: shell injection prevention', () => {
  it('does not interpret shell metacharacters', async () => {
    const result = await runCommand('echo', { binary: 'echo' }, ['$HOME', '&&', 'ls'], undefined, undefined, makeRuntimeCtx());
    expect((result as { stdout: string }).stdout.trim()).toBe('$HOME && ls');
  });

  it('does not execute command injection via backticks', async () => {
    const result = await runCommand('echo', { binary: 'echo' }, ['`whoami`'], undefined, undefined, makeRuntimeCtx());
    expect((result as { stdout: string }).stdout.trim()).toBe('`whoami`');
  });

  it('does not execute pipe attempts', async () => {
    const result = await runCommand('echo', { binary: 'echo' }, ['test', '|', 'grep', 'test'], undefined, undefined, makeRuntimeCtx());
    expect((result as { stdout: string }).stdout.trim()).toBe('test | grep test');
  });
});

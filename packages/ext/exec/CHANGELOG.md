...
- With `inheritEnv: false` the child no longer receives the parent environment. Only `PATH` is kept so `execFile` can still resolve bare-name binaries such as `git`. ([#88](https://github.com/rcrsr/rill-ext/issues/88), [#102](https://github.com/rcrsr/rill-ext/pull/102))

...
- Responses-API `message()` issues one request per call (was two) through a single `responses.stream()` runner read via `finalResponse()`; `dispose()` aborts it. ([#95](https://github.com/rcrsr/rill-ext/issues/95), [#102](https://github.com/rcrsr/rill-ext/pull/102))

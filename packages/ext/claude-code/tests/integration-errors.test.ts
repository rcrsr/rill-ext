...
describe('EC-3, AC-11: Empty text to prompt', () => {
  it('throws RuntimeError RILL-R004 "prompt text cannot be empty"', async () => {
    ...
    await expect((ext.value as any).prompt.fn({ text: '', options: {} }, ctx)).rejects.toThrow(RuntimeError);

    await expect((ext.value as any).prompt.fn({ text: '', options: {} }, ctx)).rejects.toThrow(
  ...
  it('throws RuntimeError for whitespace-only text', async () => {
    ...
    await expect((ext.value as any).prompt.fn({ text: '\t\n  ', options: {} }, ctx)).rejects.toThrow(
      'prompt text cannot be empty'
    );
  });
});
...
describe('EC-4, AC-6: Binary not found at spawn (ENOENT)', () => {
  it('throws RuntimeError RILL-R004 "claude binary not found"', async () => {
    ...
    await expect((ext.value as any).prompt.fn({ text: 'test', options: {} }, ctx)).rejects.toThrow(
      RuntimeError
    );
...
describe('EC-10: Empty skill name', () => {
  it('throws RuntimeError RILL-R004 "skill name cannot be empty"', async () => {
    ...
    await expect((ext.value as any).skill.fn({ name: '', args: {} }, ctx)).rejects.toThrow(RuntimeError);

    await expect((ext.value as any).skill.fn({ name: '', args: {} }, ctx)).rejects.toThrow(
...
describe('EC-11: Invalid skill name (non-zero exit)', () => {
  it('throws RuntimeError RILL-R004 "Claude CLI exited with code {N}"', async () => {
    ...
    await expect((ext.value as any).skill.fn({ name: 'invalid-skill', args: {} }, ctx)).rejects.toThrow(
      RuntimeError
    );
...
describe('EC-12: Skill spawn/parse/timeout errors', () => {
  it('throws RuntimeError for spawn error (same as prompt)', async () => {
    ...
    await expect((ext.value as any).skill.fn({ name: 'test-skill', args: {} }, ctx)).rejects.toThrow(
      /Failed to spawn claude binary/
    );
  });
...
describe('EC-13: Empty command name', () => {
  it('throws RuntimeError RILL-R004 "command name cannot be empty"', async () => {
    ...
    await expect((ext.value as any).command.fn({ name: '', args: {} }, ctx)).rejects.toThrow(RuntimeError);

    await expect((ext.value as any).command.fn({ name: '', args: {} }, ctx)).rejects.toThrow(
  ...
  it('throws RuntimeError for whitespace-only command name', async () => {
    ...
    await expect((ext.value as any).command.fn({ name: '\t\n', args: {} }, ctx)).rejects.toThrow(
      'command name cannot be empty'
    );
  });
});
...
describe('EC-14: Invalid command (non-zero exit)', () => {
  it('throws RuntimeError RILL-R004 "Claude CLI exited with code {N}"', async () => {
    ...
    await expect((ext.value as any).command.fn({ name: 'invalid-command', args: {} }, ctx)).rejects.toThrow(
      RuntimeError
    );
...
describe('EC-15: Command spawn/parse/timeout errors', () => {
  it('throws RuntimeError for spawn error (same as prompt)', async () => {
    ...
    await expect((ext.value as any).command.fn({ name: 'test-command', args: {} }, ctx)).rejects.toThrow(
      /claude binary not found/
    );
  });
...
describe('EC-16: Cleanup failure on dispose', () => {
  it('logs warning and does not throw when dispose cleanup fails', async () => {
    ...
    const promptPromise = (ext.value as any).prompt.fn({ text: 'test', options: {} }, ctx);
  ...
  it('continues cleanup for all disposers even if one fails', async () => {
    ...
    const promise1 = (ext.value as any).prompt.fn({ text: 'test1', options: {} }, ctx);
    const promise2 = (ext.value as any).prompt.fn({ text: 'test2', options: {} }, ctx);

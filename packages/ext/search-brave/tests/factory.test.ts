...
describe('createBraveExtension', () => {
  describe('configuration validation', () => {
    ...
    it('accepts valid config with apiKey only [AC-1]', () => {
      expect(() =>
        createBraveExtension({ apiKey: 'brave-test-key' }, makeFactoryCtx())
      ).not.toThrow();
    });
  ...
  describe('factory shape [AC-1]', () => {
    ...
    it('news has query (string) param and options (dict) param', () => {
      ...
      expect(news.params[0]).toMatchObject({
        name: 'query',
        type: { kind: 'string' },
      });
      expect(news.params[1]).toMatchObject({
        name: 'options',
        type: { kind: 'dict' },
      });
    });
    ...
    it('summarize has query (string) param and options (dict) param', () => {
      ...
      expect(summarize.params[0]).toMatchObject({
        name: 'query',
        type: { kind: 'string' },
      });
      expect(summarize.params[1]).toMatchObject({
        name: 'options',
        type: { kind: 'dict' },
      });
    });

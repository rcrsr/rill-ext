...
async function callDrive(
  ...
  return getCallable(ext, name).fn(args as Record<string, import('@rcrsr/rill').RillValue>, createRuntimeContext());
}
...
describe('drive_list success cases', () => {
  it('returns { files: list[dict] } with file entries [AC-12]', async () => {
    ...
    const result = await callDrive(ext, 'drive_list', {}) as { files: unknown[] };
  ...
  it('AC-13: emits "google:drive:list" event with duration on success', async () => {
    ...
    await getCallable(ext, 'drive_list').fn({} as Record<string, import('@rcrsr/rill').RillValue>, ctx);
...
describe('drive_upload success cases', () => {
  it('returns dict with id, name, mimeType, size, owner [AC-12]', async () => {
    ...
    const [calledUrl, calledInit] = (globalThis.fetch as unknown as ReturnType<typeof vi.fn>)
  ...
  it('AC-13: emits "google:drive:upload" event with duration on success', async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ id: 'u1', name: 'f.txt', mimeType: 'application/octet-stream' }),
    }) as typeof fetch;

    await getCallable(ext, 'drive_upload').fn(
      { content: BASE64_100, filename: 'f.txt' } as Record<string, import('@rcrsr/rill').RillValue>,
      ctx
    );
...
describe('drive_get_metadata success cases', () => {
  ...
  it('AC-13: emits "google:drive:get_metadata" event on success', async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ id: 'f1', name: 'doc.txt', mimeType: 'text/plain' }),
    }) as typeof fetch;
...
describe('EC-7: allowedFolderIds restriction', () => {
  const config = {
    auth: { type: 'bearer' as const, token: 'test-token' },
    capabilities: {
      drive: { read: true, list: true, upload: true, download: true, share: true, delete: true },
    },
...
expect(getStatus(caught as RillValue).message).toBe(
  ...
  it('BC-8: drive_upload with exactly 100-byte content → succeeds (inclusive boundary)', async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ id: 'u1', name: 'ok.bin', mimeType: 'application/octet-stream' }),
    }) as typeof fetch;

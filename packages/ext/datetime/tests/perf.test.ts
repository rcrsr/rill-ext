...
describePerf('performance', () => {
  ...
  describe('AC-NF1: time::format', () => {
    it('P99 latency is below 1 ms for 1000 sequential calls', async () => {
      ...
      const formatFn = (
        ext.value.format as { fn: (args: Record<string, unknown>) => Promise<unknown> }
      ).fn;
      ...
      for (let i = 0; i < CALL_COUNT; i++) {
        ...
        await formatFn({ dt: DT_FIXED, pattern: PATTERN });
  ...
  describe('AC-NF2: time::parse', () => {
    it('P99 latency is below 1 ms for 1000 sequential calls', async () => {
      const ext = createDatetimeExtension();
      const parseFn = (
        ext.value.parse as { fn: (args: Record<string, unknown>) => Promise<unknown> }
      ).fn;
      ...
      for (let i = 0; i < CALL_COUNT; i++) {
        const start = performance.now();
        await parseFn({ str: FORMATTED_STR, pattern: PATTERN });
        durations.push(performance.now() - start);
      }

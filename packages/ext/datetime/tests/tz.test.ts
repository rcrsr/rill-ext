...
describe('tz::iso', () => {
  it('AC-1: converts UTC 06:00Z to America/New_York EST 01:00:00-05:00', async () => {
    const ext = getExt();
    const result = await getIso(ext)(
      { dt: DT_EST, zone: 'America/New_York' },
      makeRuntimeCtx()
    );
  ...
  it('AC-B1: DST transition instant 2026-03-08T07:00:00Z resolves to EDT -04:00', async () => {
    const ext = getExt();
    const result = await getIso(ext)(
      { dt: DT_DST_TRANSITION, zone: 'America/New_York' },
      makeRuntimeCtx()
    );
  ...
  it('AC-B1: one second before spring-forward resolves to EST -05:00', async () => {
    const ext = getExt();
    const result = await getIso(ext)(
      { dt: DT_DST_BEFORE, zone: 'America/New_York' },
      makeRuntimeCtx()
    );
  ...
  it('AC-B2: epoch datetime in Africa/Abidjan produces 1970-01-01T00:00:00+00:00', async () => {
    const ext = getExt();
    const result = await getIso(ext)(
      { dt: DT_EPOCH, zone: 'Africa/Abidjan' },
      makeRuntimeCtx()
    );
  ...
  it('AC-E5: string dt argument returns invalid value', async () => {
    const ext = getExt();
    const result = await getIso(ext)(
      { dt: '2026-03-08T06:00:00Z', zone: 'America/New_York' },
      makeRuntimeCtx()
    );
    const status = getStatus(result);
    expect(status.code.name).toBe('INVALID_INPUT');
  ...
  it('AC-E5: object dt argument returns invalid value', async () => {
    const ext = getExt();
    const result = await getIso(ext)(
      { dt: { value: 0 }, zone: 'America/New_York' },
      makeRuntimeCtx()
    );
    const status = getStatus(result);
    expect(status.code.name).toBe('INVALID_INPUT');
...
describe('tz::date', () => {
  it('AC-2: UTC 2026-04-01T23:00:00Z + Asia/Tokyo crosses midnight to 2026-04-02', async () => {
    const ext = getExt();
    const result = await getDate(ext)(
      { dt: DT_2026_04_01_23, zone: 'Asia/Tokyo' },
      makeRuntimeCtx()
    );
...
describe('tz::time', () => {
  it('AC-3: produces correct HH:mm:ss for America/New_York EST', async () => {
    const ext = getExt();
    const result = await getTime(ext)(
      { dt: DT_EST, zone: 'America/New_York' },
      makeRuntimeCtx()
    );
...
describe('tz::offset', () => {
  it('AC-4: Asia/Calcutta without dt returns typeof number', async () => {
    const ext = getExt();
    const result = await getOffset(ext)(
      { zone: 'Asia/Calcutta' },
      makeRuntimeCtx()
    );
  ...
  it('AC-5: America/New_York with summer datetime returns -4', async () => {
    const ext = getExt();
    const result = await getOffset(ext)(
      { dt: DT_NY_SUMMER, zone: 'America/New_York' },
      makeRuntimeCtx()
    );
  ...
  it('AC-5: America/New_York with winter datetime returns -5', async () => {
    const ext = getExt();
    const result = await getOffset(ext)(
      { dt: DT_NY_WINTER, zone: 'America/New_York' },
      makeRuntimeCtx()
    );
  ...
  it('AC-B5: Asia/Calcutta returns 5.5', async () => {
    const ext = getExt();
    const result = await getOffset(ext)(
      { dt: DT_2026_JAN, zone: 'Asia/Calcutta' },
      makeRuntimeCtx()
    );
  ...
  it('AC-B6: Asia/Katmandu returns 5.75 for a 2026 instant', async () => {
    const ext = getExt();
    const result = await getOffset(ext)(
      { dt: DT_2026_JAN, zone: 'Asia/Katmandu' },
      makeRuntimeCtx()
    );
  ...
  it('AC-E5: non-datetime dt with valid zone returns invalid value', async () => {
    const ext = getExt();
    const result = await getOffset(ext)(
      { zone: 'America/New_York', dt: 'yesterday' },
      makeRuntimeCtx()
    );
    const status = getStatus(result);
    expect(status.code.name).toBe('INVALID_INPUT');
...
describe('tz::zones', () => {
  ...
  it('AC-6: sample zones from zones() are accepted by tz::iso without error', async () => {
    ...
    for (const i of indices) {
      ...
      await expect(
        isoFn({ dt: DT_2026_JAN, zone }, makeRuntimeCtx())

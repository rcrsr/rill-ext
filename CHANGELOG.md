...
- **Node floor raised to `>=22.16.0`:** `@rcrsr/rill@0.20.0` declares `engines.node` as `>=22.16.0`, so the workspace root and `@rcrsr/rill-ext-text` (the only packages declaring `engines`) bump from `>=22.0.0` to match. Any environment able to install rill `0.20.0` already satisfies this floor.
...
- Rich `returnType` shapes declared across 12 extensions per the new policy `.claude/policies/policy-domain-ext.md` §EXT.8. Every host-function callable that returns a known-schema response now exposes the precise `dict(fields:)` or `list(element:)` shape via `structureToTypeValue`, replacing the previous shapeless `dict` / `list`. `anyTypeValue` is retained only for the four §EXT.8.3 cases (user-stored values, externally-supplied tool schemas, untyped HTTP bodies, heterogeneous runtime state). Scripts introspecting any updated callable's `returnType` property now see the precise shape instead of bare `dict` / `list` / `any`.
  - `@rcrsr/rill-ext-search-brave` bumped to `0.19.1`. 3 callables (`search`, `news`, `summarize`) typed; inner vendor objects retain `any`.
  - `@rcrsr/rill-ext-search-exa` bumped to `0.19.1`. 4 callables typed; `search` and `find_similar` share an identical shape.
  - `@rcrsr/rill-ext-search-serper` bumped to `0.19.1`. 3 callables typed; image element fields stay camelCase pending a separate boundary fix.
  - `@rcrsr/rill-ext-search-searxng` bumped to `0.19.1`. 2 callables typed.
  - `@rcrsr/rill-ext-search-tavily` bumped to `0.19.1`. 2 callables typed; `answer` and `images` declared as optional fields gated on the `include_*` flags.
  - `@rcrsr/rill-ext-google-workspace` bumped to `0.19.4`. 8 dict callables typed; drive file fields shared between `drive_list` and `drive_get_metadata`; calendar event fields shared between `calendar_events` and `calendar_today`; `calendar_free_busy` types as the homogeneous email-keyed dict the code actually returns. Folded snake_case boundary fixes: `calendar_today` attendees `displayName`/`responseStatus` → `display_name`/`response_status`; `gmail_read` attachment `mimeType` → `mime_type`. Reconciled `docs/README.md` with code (gmail_read headers nested + attachments row + stale snippet/labels removed; drive_upload size/owner rows added; calendar_* event element shape expanded; calendar_free_busy flat dict replaces the prior `{schedules: dict}` claim).
  - `@rcrsr/rill-ext-outlook` bumped to `0.19.2`. 12 callables typed via `MailMessageDict` / `CalendarEventDict` / `FreeBusyScheduleDict` shared element constants. Folded 4 snake_case boundary fixes: `MailMessageDict.hasAttachments` → `has_attachments`, `CalendarEventDict.isOnline` → `is_online`, `CalendarEventDict.onlineUrl` → `online_url`, `FreeBusyScheduleDict.scheduleId` → `schedule_id`; `create_event` options `isOnline` → `is_online`. Reconciled docs: `flag` Result Dict rewritten to cross-reference Message Dict Shape (was wrongly claiming `{id, flagged}`); new Calendar Event Dict Shape section added.
  - `@rcrsr/rill-ext-kv-redis`, `@rcrsr/rill-ext-kv-sqlite`, `@rcrsr/rill-ext-kv-file` each bumped to `0.19.1`. `getAll` typed as `dict(string: any)` (homogeneous-value form); `get` / `get_or` keep `any` per §EXT.8.3 case 1.
  - `@rcrsr/rill-ext-fetch` bumped to `0.19.1`. Dynamic endpoints with `responseShape: 'full'` typed as `dict(status, headers, body)`; `'body'` shape stays `any` per §EXT.8.3 case 3; `endpoints` introspection helper typed as `list(dict(name, method, path, description))`.
  - `@rcrsr/rill-ext-exec` bumped to `0.19.2`. Per-command callable typed as `dict(stdout, stderr, exit_code)`; `commands` introspection helper typed as `list(dict(name, description))`.
  - `@rcrsr/rill-ext-fs-local` bumped to `0.19.1`. `mounts` introspection helper typed as `list(dict(name, mode, glob))`.
  - `@rcrsr/rill-ext-mcp` bumped to `0.19.1`. The three resource read callables are typed as `any` per §EXT.8.3 case 4 (resource content schema is set by the MCP server at runtime); tool callables (with `outputSchema`) and prompt callables already declared concrete shapes.
